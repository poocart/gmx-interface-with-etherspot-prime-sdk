...
export default function SwapBox(props) {
  ...
  const addEtherspotPrimeTransaction = async (transaction) => {
    const etherspotPrimeSdk = await getEtherspotPrimeSdkForChainId(42161);
  ...
  const getLeverageError = () => {
    ...
    if (!hasExistingPosition && fromUsdMin && fromUsdMin.lt(expandDecimals(0.1, USD_DECIMALS))) {
      return [t`Min order: 0.1 USD`];
    }
